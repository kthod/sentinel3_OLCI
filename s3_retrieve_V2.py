...
def retr(p, n, target_lat, target_lon):
    ...
    return(n + ', ' + elements[11] + ',' + elements[12] + ',' + str(date) + ', ' + str(target_x_y) + ', ' + str(target_lon) + ', ' + str(target_lat) + ', '  + str(iwv_value) + ', ' + str(iwv_err_value) + ', ' + str(iwv_avg) + ', ' + str(perc_excluded)) 

# Create the csv file
f = open("s3A_lost_cycles.csv", "w")
f.write("File, Cycle, Orbit, Date, Time, image_coords, Longitude, Latitude, IWV, IWV error, IWV_average, Excluded_pixels (%),   \n")

for root, dirs, files in os.walk('/media/server/My Passport/gpetr/sentinel3_images/S3A_lost_cycles'):
    for name in dirs:
        # In this step we are in an netcdf image folder
        path = os.path.join(root, name)
        with open('locations2.txt', 'r') as f_loc:
