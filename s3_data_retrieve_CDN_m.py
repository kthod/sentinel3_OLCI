...
def retr(p, n, target_lat, target_lon):
    ...
    nc_inst = Dataset(p + '/instrument_data.nc', 'r')

    frame_offset = nc_inst.variables['detector_index'][:]
    frame_offset = frame_offset.data
    ...
    perc_excluded = (100 * excluded_pixels) / ((up+down)*(left+right))
    ...
    time_stamp = time[target_x_y[0] - 1] - frame_offset[target_x_y]*0.044  # because python array index start from 0 while sentinel rows from 1
    date = dt(2000, 1, 1, 0, 0 ,0) + timedelta(seconds=time_stamp/1e6)
    
    return(n + ', ' + elements[11] + ', ' + elements[12] + ', ' + str(date) + ', ' + str(target_x_y[0]) + ', '+ str(target_x_y[1]) + ', ' + str(target_lon) + ', ' + str(target_lat) + ', '  + str(iwv_value) + ', ' + str(iwv_avg) + ', ' + str(perc_excluded) + ', ' + str(iwv_err_value)) 
...
for root, dirs, files in os.walk('C:/Users/thodoris/Documents/Python_Scripts/s3/s3/data2'):
    for name in dirs:
