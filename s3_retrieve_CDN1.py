...
# Create the csv file
f = open("s3A_values_CDN1.csv", "w")
f.write("File, Cycle, Orbit, Date, Time, image_coords, Longitude, Latitude, IWV, IWV_average, Excluded_pixels (%), IWV error   \n")

for root, dirs, files in os.walk('/media/server/TOSHIBA EXT/OLCI_CDN1'):
    for name in dirs:
        # In this step we are in an netcdf image folder
        path = os.path.join(root, name)
        with open('CDN1.txt', 'r') as f_loc:
