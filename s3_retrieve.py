...
# Create the csv file
f = open("s3A_values_orbit178_rodak.csv", "w")
f.write("File, Cycle, Orbit, Date, Time, image_coords, Longitude, Latitude, IWV, IWV error \n")

for root, dirs, files in os.walk('/media/server/My Passport/gpetr/sentinel3_images/S3A/178'):
    for name in dirs:
        # In this step we are in an netcdf image folder
        path = os.path.join(root, name)
        with open('location_rodakino.txt', 'r') as f_loc:
